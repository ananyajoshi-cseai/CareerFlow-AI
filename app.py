...
if selected == "Scanner":
    ...
    with st.container(border=True):
        ...
        with c2:
            ...
            if st.button("🧠 Analyze JD with AI"):
                if not st.session_state.api_key: st.error("Need API Key")
...
# === PAGE 2: BATTLE MODE (VISUAL EDITION) ===
    ...
    st.markdown('<h1 style="color:#00C9FF; text-align: center;">⚔️ Job Battle Arena ⚔️</h1>', unsafe_allow_html=True)
    ...
    if st.button("🔥 Fight! (Compare Jobs)"):
        if not st.session_state.api_key:
            st.error("⚠️ Enter API Key first!")
        elif not jd_a or not jd_b:
            st.warning("⚠️ Please paste both JDs.")
        else:
            with st.spinner("⚔️ Judging the duel..."):
                # 1. THE PROMPT
                prompt = f"""
                ...
                try:
                    ...
                    st.markdown("### 🕸️ Skill & Benefit Face-off")
...
# === PAGE 3: ANALYTICS ===
    ...
    conn = sqlite3.connect('career_data.db')
    an_tab1, an_tab2 = st.tabs(["📈 Dashboard", "🗂️ Manage Data"])
...
# === PAGE 4: TOOLS ===
    ...
    if not st.session_state.resume_text: st.warning("⚠️ Upload resume in 'Scanner' first!")
    else:
        t1, t2, t3 = st.tabs(["📝 Cover Letter", "🔎 Resume Critique", "🤖 Interview Prep"])
        with t1:
            ...
            if st.session_state.cover_letter:
                ...
                st.download_button("📄 Download PDF", data=pdf_bytes, file_name="cover_letter.pdf", mime="application/pdf")
        with t2:
